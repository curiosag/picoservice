...
public class ServiceTest {
    ...
    */
    ...
    public void testInitializeAndTerminateComputation_ConstValuesOnly() {
        ...
        resultListener.initAndFinalize(add);
        ...
        resultListener.recieve(message(Name.initializeComputation, null, source));
    ...
    public void testLet() {
        ...
        */
        ...
        functionCallTarget
                .initAndFinalize(constOne) // const must be dependent too but needs no propagations
                .initAndFinalize(add)
                .initAndFinalize(mul)
        ...
        Function<Integer> functionCall = call(functionCallTarget)
                .returnTo(resultListener, Name.result);
        resultListener.initAndFinalize(functionCall);
        resultListener.param(Name.result);

        resultListener.recieve(message(Name.initializeComputation, null, createSource(resultListener)));
    ...
    public void testFunctionCallNoParams() {
        ...
        resultReceiver.initAndFinalize(caller);

        caller.initAndFinalize(callee);
        callee.initAndFinalize(constOne);

        resultReceiver.param(Name.result);

        resultReceiver.recieve(message(Name.initializeComputation, null, createSource(resultReceiver)));
        await(() -> result.value != 0);
    ...
    public void testFunctionCall() {
        ...
        Function<Integer> add = add();
        Function<Integer> callTarget = callTarget(add);
        callTarget.propagate(Name.a, Name.leftArg, add);
        callTarget.propagate(Name.b, Name.rightArg, add);


        Function<Integer> callAddA = call(callTarget);
        Function<Integer> callAddB = call(callTarget);
        ...
        callAddA.returnTo(resultReceiver, resultCallA);
        callAddB.returnTo(resultReceiver, resultCallB);

        resultReceiver.param(resultCallA);
        resultReceiver.param(resultCallB);

        Source sourceA = source(resultReceiver, 0L, 0);
        callAddA.recieve(message(Name.a, 1, sourceA));
        callAddA.recieve(message(Name.b, 2, sourceA));

        Source sourceB = source(resultReceiver, 1L, 0);
        callAddB.recieve(message(Name.a, 3, sourceB));
        callAddB.recieve(message(Name.b, 4, sourceB));
    ...
    public void testIf() {
        ...
        resultReceiver.initAndFinalize(_if);
        _if.initAndFinalize(gt);
        _if.initAndFinalize(subT);
        _if.initAndFinalize(subF);

        // True-path
        Source sourceA = source(resultReceiver, 0L, 0);
        resultReceiver.recieve(message(Name.initializeComputation, null, sourceA));
        ...
        result.setValue(0);
        Source sourceB = source(resultReceiver, 1L, 0);
        resultReceiver.recieve(message(Name.initializeComputation, null, sourceB));
    ...
    public void testSequentialIf() {
        ...
        iff.initAndFinalize(gt);
        // true and false branches must propagate, init and finalize differently, and also separately from the condition
        iff.addPropagationOnTrue(Name.a, Name.leftArg, subT);
        iff.addPropagationOnTrue(Name.b, Name.rightArg, subT);
        iff.addInitAndFinalizeOnTrue(subT);

        iff.propagateOnFalse(Name.b, Name.leftArg, subF);
        iff.propagateOnFalse(Name.a, Name.rightArg, subF);
        iff.initAndFinalizeOnFalse(subF);

        resultReceiver.initAndFinalize(iff);
        ...
        resultReceiver.recieve(message(Name.initializeComputation, null, computationA));
        ...
        resultReceiver.recieve(message(Name.initializeComputation, null, computationB));
    ...
    public void testRecursion() {
        ...
        resultMonitor.initAndFinalize(sumCall);
        sumCallTarget.initAndFinalize(_if);
        _if.initAndFinalize(eq);
        _if.initAndFinalizeOnFalse(add);
        add.initAndFinalize(sumCallRec);
        sumCallRec.initAndFinalize(sub);
